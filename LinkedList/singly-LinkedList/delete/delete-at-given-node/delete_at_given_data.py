"""Deleting at the given value of the LinkedList :
...
class LinkedList:
    ...
    def delete_at_given_value(self, x):
        if self.head is None:
            print('LinkedList is empty!!')
            return

        """Checking if it is first Node"""
        if x == self.head.data:
            """if it first Node , to delete it we have to set the head to the second Node(coz first node reference  contains the 2nd Node)"""
        ...
        if n.ref is None:
            ...
            """getting the previous node and setting the the reference of next to next node to the prev node"""
            n.ref = n.ref.ref
...
ll.adding_element_at_begining(40)
ll.adding_element_at_begining(80)
ll.delete_at_given_value(10)
ll.print_LinkedList()
